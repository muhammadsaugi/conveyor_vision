...
# Stack: streamlit==1.57.0, streamlit-webrtc==0.64.6,
...
# ── Import av + streamlit_webrtc dalam SATU blok ─────────────
# PENTING: kedua import harus dalam satu try agar
# WEBRTC_AVAILABLE tidak di-override jika salah satu gagal
try:
    import av
...
def annotate_frame(frame, results, thresh, line_y, state):
    ...
    if results and results[0].boxes is not None:
        for box in results[0].boxes:
            ...
            cv2.rectangle(frame, (x1, y1), (x2, y2), color,
                          3 if label == "Tidak Dikenali" else 2)
            ...
            cv2.putText(frame, txt, (x1+3, y1-3),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.52, (255, 255, 255), 1, cv2.LINE_AA)
            cv2.circle(frame, (cx, cy), 5, color, -1)

    # HUD counter
    hud = [
        ("Good Day",     state["count"]["Good Day"],      (100, 220, 100)),
    ...
    for i, (lbl, cnt, clr) in enumerate(hud):
        cv2.putText(frame, f"{lbl:<14}: {cnt:>3}",
                    (8, 22 + i * 26), cv2.FONT_HERSHEY_SIMPLEX,
                    0.52, clr, 1, cv2.LINE_AA)
    cv2.putText(frame, f"{'Total':<14}: {sum(state['count'].values()):>3}",
                (8, 22 + 3 * 26), cv2.FONT_HERSHEY_SIMPLEX,
...
def save_log():
    ...
    with open(sp, "w", encoding="utf-8") as f:
        json.dump({
            ...
            "counters": {
                "good_day"      : st.session_state["count"]["Good Day"],
...
with st.sidebar:
    ...
    if not YOLO_AVAILABLE:
        st.error(f"❌ YOLO gagal: {_YOLO_ERROR}")
    ...
    if not WEBRTC_AVAILABLE:
        st.error(f"❌ WebRTC gagal: {_WEBRTC_ERROR}")
    else:
        st.success("✅ WebRTC tersedia")
    ...
    st.caption(
        ...
        f"**WebRTC:** {'✅' if WEBRTC_AVAILABLE else '❌'}  "
...
st.markdown(
    ...
    "<p style='color:gray;margin-top:0'>Deteksi real-time "
    "Good Day &amp; Nutrisari — Sistem Pemilahan Sampah</p>",
...
alert_now = (st.session_state["alert_active"]
             and time.time() < st.session_state["alert_until"])
...
with c5:
    if alert_now:
        st.error("🚨 ALERT!")
    elif cnt["Tidak Dikenali"] > 0:
        st.warning(f"⚠️ {cnt['Tidak Dikenali']} tdk dikenali")
    else:
        st.success("✅ Normal")
...
if not WEBRTC_AVAILABLE:
    ...
    st.error(
        ...
        f"Detail error: `{_WEBRTC_ERROR}`  \n\n"
        "Coba refresh halaman. Jika masih error, hubungi developer."
    )
elif not YOLO_AVAILABLE:
    st.error(f"❌ YOLO tidak tersedia: `{_YOLO_ERROR}`")
else:
    ...
    if not ctx.state.playing:
        st.info(
            ...
            "📱 **HP**: Buka di Chrome/Safari → izinkan kamera  \n"
            "💻 **Laptop**: Gunakan Chrome/Edge → izinkan kamera  \n"
            "🔒 Butuh koneksi **HTTPS** (Streamlit Cloud sudah HTTPS ✅)"
        )
...
with tab2:
    if st.session_state["detections"]:
        ...
        st.dataframe(
            df_f[["id", "track_id", "label", "confidence", "timestamp"]],
            hide_index=True,
            use_container_width=True,
            column_config={
...
- **Kamera tidak muncul di HP?** → Buka di Chrome → Allow camera
- **Kamera tidak muncul di Laptop?** → Gunakan Chrome/Edge
    """)